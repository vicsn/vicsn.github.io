(() => {
  ...
  const i18n = {
    en: {
      ...
      'match.emptyTitle':'No matches yet.','match.emptyText':'Complete your profile or ask an administrator to run matching.','match.profile':'Complete profile','match.pageEyebrow':'MY MATCHES','match.pageTitle':'Your Lunchclub matches','match.pageLead':'See your current introduction and a concise record of past conversations.','match.eyebrow':'CURRENT MATCH','match.title':'You have a conversation to look forward to.','match.choose':'Choose a meeting time','match.chooseText':'These options are generated from the shared availability in test mode.','match.topicsLabel':'SUGGESTED CONVERSATION STARTERS','match.topicsTitle':'Start with what you already have in common.','match.feedbackTitle':'After the conversation','match.feedbackText':'Use this demo form to simulate the follow-up email.','match.ratePrompt':'How useful was this conversation?','match.notHeld':'Meeting did not happen','match.useful':'Meeting happened & was useful','match.historyEyebrow':'MATCH HISTORY','match.historyTitle':'Previous matches','match.historyMissed':'Did not happen','match.historyUseful':'Useful',
    ...
    de: {
      'nav.home':'Start','nav.how':'So funktioniert’s','nav.profile':'Mein Profil','nav.match':'Meine Matches','nav.admin':'Admin','nav.signin':'Demo-Anmeldung','nav.join':'Lunchclub beitreten',
      'test.banner':'Lokale Demo. E-Mail-, Kalender-, Mitgliedschafts- und Datenbank-Integrationen werden simuliert.',
      'home.eyebrow':'BWK MITGLIEDERNETZWERK','home.title':'Treffen Sie die richtige Person für Ihr nächstes Fachgespräch.','home.lead':'Lunchclub bringt neue und etablierte BWK-Mitglieder für fokussierte Gespräche zu zweit oder zu dritt zusammen – passend zu Zielen, Verfügbarkeit und beruflichem Hintergrund.','home.cta':'Mein Match finden','home.secondary':'So funktioniert’s →','home.trust1':'Verifizierte Mitglieder','home.trust2':'Intelligentes Matching','home.trust3':'Einfache Terminfindung','home.card.you':'IHR ZIEL','home.card.goal':'Wissen austauschen','home.card.profile':'Wasserwirtschaft · Berufseinstieg','home.card.match':'MATCH','home.card.matchProfile':'Hochwasserschutz · 18 Jahre Erfahrung','home.card.fit':'Match-Qualität','home.whyEyebrow':'FÜR NÜTZLICHE KONTAKTE','home.whyTitle':'Weniger Hürden beim Netzwerken. Mehr relevante Gespräche.','home.whyText':'Sagen Sie uns, was Sie lernen, weitergeben oder erkunden möchten. Lunchclub übernimmt die Vorstellung und bietet direkt passende Termine an.','home.feature1.title':'Kurzen Fragebogen ausfüllen','home.feature1.text':'Hintergrund, Ziele, Verfügbarkeit und bevorzugte Gesprächsgröße angeben.','home.feature2.title':'Passendes Match erhalten','home.feature2.text':'Zuerst prüfen wir Verfügbarkeit und Gruppengröße, danach Alter, Hintergrund und Ziele.','home.feature3.title':'Von der Vorstellung zum Treffen','home.feature3.text':'Persönliche Vorstellung, Gesprächsthemen und Terminoptionen – alles in einem klaren, einfachen Ablauf.','home.quote':'Ein starkes berufliches Netzwerk sollte leicht zugänglich sein – besonders für Menschen, die neu in der Gemeinschaft sind.','home.quoteSource':'Lunchclub-Prinzip',
      ...
      'match.emptyTitle':'Noch keine Matches.','match.emptyText':'Vervollständigen Sie Ihr Profil oder lassen Sie das Matching durch einen Admin starten.','match.profile':'Profil vervollständigen','match.pageEyebrow':'MEINE MATCHES','match.pageTitle':'Ihre Lunchclub-Matches','match.pageLead':'Aktuelle Vorstellung und eine kurze Übersicht vergangener Gespräche.','match.eyebrow':'AKTUELLES MATCH','match.title':'Ein interessantes Gespräch wartet auf Sie.','match.choose':'Termin auswählen','match.chooseText':'Diese Optionen werden im Testmodus aus der gemeinsamen Verfügbarkeit erzeugt.','match.topicsLabel':'VORGESCHLAGENE GESPRÄCHSSTARTER','match.topicsTitle':'Beginnen Sie mit Ihren gemeinsamen Themen.','match.feedbackTitle':'Nach dem Gespräch','match.feedbackText':'Mit diesem Demo-Formular wird die Follow-up-E-Mail simuliert.','match.ratePrompt':'Wie nützlich war das Gespräch?','match.notHeld':'Treffen fand nicht statt','match.useful':'Treffen fand statt & war nützlich','match.historyEyebrow':'MATCH-HISTORIE','match.historyTitle':'Frühere Matches','match.historyMissed':'Nicht stattgefunden','match.historyUseful':'Nützlich',
  ...
  let pendingUsefulnessRating = false;
  ...
  function signOut() {
    currentUserId = null; sessionStorage.removeItem(SESSION_KEY); pendingUsefulnessRating = false; renderAuth(); route('home'); toast(lang==='de'?'Abgemeldet.':'Signed out.');
  }
  ...
  function historicalMatchesFor(user) {
    return state.matches.filter(m => m.memberIds.includes(user.id) && ['completed', 'failed'].includes(m.status))
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }
  function feedbackForMatch(matchId, userId) {
    return state.feedback.find(f => f.matchId === matchId && f.userId === userId);
  }
  function matchPartnerLabel(match, user) {
    return match.memberIds.map(id => state.members.find(m => m.id === id)).filter(m => m && m.id !== user.id)
      .map(m => `${m.firstName} ${m.lastName}`).join(', ');
  }
  function matchWhenLabel(match) {
    const when = match.selectedDate || match.createdAt;
    return new Date(when).toLocaleDateString(lang === 'de' ? 'de-DE' : 'en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
  }
  function deactivateAfterMatch(user) {
    if (user.access === 'test') user.active = false;
    if (user.profile?.frequency === 'once') user.active = false;
  }
  function submitUsefulnessRating(rating) {
    const user = currentUser();
    const match = currentMatchFor(user);
    if (!user || !match) return;
    match.status = 'completed';
    state.feedback.push({ matchId: match.id, userId: user.id, useful: true, rating, createdAt: new Date().toISOString() });
    pendingUsefulnessRating = false;
    deactivateAfterMatch(user);
    saveState();
    toast(lang === 'de' ? `Danke. Nützlichkeit ${rating}/10 gespeichert.` : `Thanks. Usefulness ${rating}/10 saved.`);
    renderMatch();
    renderAuth();
  }
  ...
  function renderMatch() {
    ...
    if (!user) return;
    const match = currentMatchFor(user);
    const history = historicalMatchesFor(user);
    const hasPage = !!(match || history.length);
    $('#matchEmpty').hidden = hasPage;
    $('#matchPage').hidden = !hasPage;
    if (!hasPage) return;

    $('#matchCurrent').hidden = !match;
    if (match) {
      const people = match.memberIds.map(id => state.members.find(m => m.id === id)).filter(Boolean);
      const others = people.filter(m => m.id !== user.id);
      const reasons = [];
      if (match.ageContrast) reasons.push(lang === 'de' ? 'Erfahrung über Generationen hinweg' : 'cross-generation experience');
      if (match.sharedGoals.length) reasons.push(`${lang === 'de' ? 'gemeinsame Ziele' : 'shared goals'}: ${match.sharedGoals.join(', ')}`);
      reasons.push(`${lang === 'de' ? 'gemeinsame Verfügbarkeit' : 'shared availability'}: ${match.sharedSlots.join(', ')}`);
      ...
      status.textContent = match.selectedDate ? (lang === 'de' ? 'Termin bestätigt' : 'Date confirmed') : (lang === 'de' ? 'Termin offen' : 'Awaiting date');
      status.classList.toggle('confirmed', !!match.selectedDate);
      const options = [];
      (match.sharedSlots.length ? match.sharedSlots : ['Tue afternoon']).forEach((slot, si) => { for (let i = 0; i < Math.min(2, 3 - options.length); i++) options.push(dateLabel(slot, i + si)); });
      ...
      const ratingPanel = $('#usefulnessRating');
      const feedbackActions = $('#feedbackActions');
      ratingPanel.hidden = !pendingUsefulnessRating;
      feedbackActions.hidden = pendingUsefulnessRating;
      if (pendingUsefulnessRating) {
        $('#ratingScale').innerHTML = Array.from({ length: 10 }, (_, i) => i + 1).map(n => `<button type="button" class="rating-btn" data-rating="${n}">${n}</button>`).join('');
      }
    } else {
      pendingUsefulnessRating = false;
    }

    const historySection = $('#matchHistory');
    historySection.hidden = !history.length;
    if (history.length) {
      $('#matchHistoryList').innerHTML = history.map(item => {
        const partners = matchPartnerLabel(item, user);
        const fb = feedbackForMatch(item.id, user.id);
        const meta = item.status === 'completed' && fb?.useful
          ? `<span class="history-tag useful">${i18n[lang]['match.historyUseful']}${fb.rating ? ` ${fb.rating}/10` : ''}</span><span>${item.score}% fit</span>`
          : `<span class="history-tag missed">${i18n[lang]['match.historyMissed']}</span>`;
        return `<article class="match-history-item"><strong>${escapeHtml(partners)}</strong><div class="match-history-meta"><span>${escapeHtml(matchWhenLabel(item))}</span>${meta}</div></article>`;
      }).join('');
    }
  }
  ...
  $$('[data-feedback]').forEach(btn => btn.addEventListener('click', () => {
    ...
    const match = currentMatchFor(user);
    if (!user || !match) return;
    if (btn.dataset.feedback === 'useful') {
      pendingUsefulnessRating = true;
      renderMatch();
      return;
    }
    match.status = 'failed';
    state.feedback.push({ matchId: match.id, userId: user.id, useful: false, createdAt: new Date().toISOString() });
    pendingUsefulnessRating = false;
    deactivateAfterMatch(user);
    saveState();
    toast(lang === 'de' ? 'Danke. Das Match wurde als nicht stattgefunden markiert.' : 'Thanks. The match was marked as not held.');
    renderMatch();
    renderAuth();
  }));
  ...
  document.addEventListener('click', e => {
    ...
    if (!ratingBtn || !pendingUsefulnessRating) return;
    submitUsefulnessRating(Number(ratingBtn.dataset.rating));
  });
