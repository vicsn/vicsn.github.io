(() => {
  ...
  const seedMembers = [
    {
      ...
      profile: { industry: 'Coastal & flood protection', experience: '16+', ageBand: '50plus', language: 'de', bio: 'I lead flood protection programmes and enjoy exchanging practical experience with younger engineers.', social: '', goals: ['Knowledge transfer','Mentoring'], groupSize: '2', frequency: '2', availability: ['Tue 12:00','Thu 12:00'] }
    },
    {
      ...
      profile: { industry: 'Water management', experience: '4-8', ageBand: '30-39', language: 'de', bio: 'I work on river restoration and want to compare project approaches across organisations.', social: '', goals: ['Industry exchange','Meet members'], groupSize: '2', frequency: '1', availability: ['Wed 17:30','Thu 12:00'] }
    },
    {
      ...
      profile: { industry: 'Public administration', experience: '16+', ageBand: '50plus', language: 'either', bio: 'My focus is public water infrastructure, teams and long-term asset planning.', social: '', goals: ['Knowledge transfer','Career development'], groupSize: '3', frequency: '1', availability: ['Wed 17:30','Fri 08:30'] }
    },
    {
      ...
      profile: { industry: 'Research & education', experience: '9-15', ageBand: '40-49', language: 'either', bio: 'I connect research with operational water quality questions and enjoy mentoring.', social: '', goals: ['Mentoring','Industry exchange'], groupSize: '3', frequency: '2', availability: ['Wed 17:30','Fri 08:30'] }
    }
  ];
  ...
  function renderQuestionnaire() {
    ...
    Object.entries(user.profile).forEach(([key,val])=>{
      if (['goals','availability'].includes(key)) { $$(`input[name="${key}"]`,form).forEach(el=>el.checked=val.includes(el.value)); }
  ...
  $('#questionnaireForm').addEventListener('submit', e => {
    ...
    if (!form.reportValidity()) return;
    const fd=new FormData(form); const goals=fd.getAll('goals'); const availability=fd.getAll('availability');
    if (!goals.length || !availability.length) { msg.textContent=lang==='de'?'Bitte mindestens ein Ziel und ein Zeitfenster wählen.':'Choose at least one goal and one availability slot.'; return; }
    const user=currentUser();
  ...
  function dateLabel(slot,index) {
    const dates=[]; const now=new Date(); let d=new Date(now); d.setDate(d.getDate()+1);
    const targetDay = { 'Tue 12:00':2,'Wed 17:30':3,'Thu 12:00':4,'Fri 08:30':5 }[slot] ?? 2;
    while(d.getDay()!==targetDay) d.setDate(d.getDate()+1); d.setDate(d.getDate()+index*7);
    const time=slot.split(' ')[1]; return {iso:`${d.toISOString().slice(0,10)}T${time}`,date:d.toLocaleDateString(lang==='de'?'de-DE':'en-GB',{weekday:'short',day:'2-digit',month:'short'}),time};
  }
  function renderMatch() {
    ...
    const options=[]; (match.sharedSlots.length?match.sharedSlots:['Tue 12:00']).forEach((slot,si)=>{ for(let i=0;i<Math.min(2,3-options.length);i++) options.push(dateLabel(slot,i+si)); });
    while(options.length<3) options.push(dateLabel(match.sharedSlots[0]||'Tue 12:00',options.length+1));
